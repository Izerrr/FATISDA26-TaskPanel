...
export function TaskNotificationTestModal({ open, onClose, currentUser }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-float">
        {/* Modal Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <form id="test-notify-form" onSubmit={handleSendTest} className="space-y-4">
            ...
            <div>
              <label htmlFor="test-title" className="mb-1.5 block text-xs font-semibold text-liquid-text">
                Judul Notifikasi
              </label>
              <input
                id="test-title"
                ...
                required
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 outline-none transition focus:border-violet-500 focus:bg-white"
              />
            </div>

            <div>
              <label htmlFor="test-desc" className="mb-1.5 block text-xs font-semibold text-liquid-text">
              ...
              <textarea
                ...
                required
                className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800 outline-none transition focus:border-violet-500 focus:bg-white"
              />
            </div>
          ...
          {clientError && (
            <div className="flex items-start gap-2.5 rounded-2xl border border-red-200 bg-red-50 p-4 text-xs text-red-700">
            ...
            >
              <div className="flex items-center gap-2 font-bold">
                {result.success ? (
                  <>
                    ...
                    <span className="text-emerald-800">Pengiriman Notifikasi Berhasil!</span>
                  </>
                ) : (
                  <>
                    <AlertTriangle className="h-4 w-4 text-red-600" />
                    <span className="text-red-800">Pengiriman Notifikasi Gagal</span>
                  </>
                )}
              </div>
              ...
              {result.diagnostics && (
                <div className="mt-3 space-y-2 border-t border-slate-200/60 pt-3">
                  ...
                  <div className="rounded-xl bg-white p-2.5 shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-2 text-[11px]">
                    <div>
                      <span className="text-slate-400 font-medium">Role Mention: </span>
                      {result.diagnostics.roleMention.included ? (
                        result.diagnostics.roleMention.configured ? (
                          <span className="font-semibold text-slate-700">
                            ID: <code className="font-mono">{result.diagnostics.roleMention.maskedRoleId}</code> (via <code className="font-mono">{result.diagnostics.roleMention.envSource}</code>)
                          </span>
                        ) : (
                          <span className="text-amber-600 font-semibold">Belum diset di .env</span>
                        )
                      ) : (
                        <span className="text-slate-400">Dimatikan</span>
                      )}
                    </div>

                    <div className="text-slate-400 text-[10px]">
                      Diuji oleh <span className="font-medium text-slate-700">{result.diagnostics.testedBy}</span> · {new Date(result.diagnostics.timestamp).toLocaleTimeString("id-ID")}
                    </div>
                  </div>
