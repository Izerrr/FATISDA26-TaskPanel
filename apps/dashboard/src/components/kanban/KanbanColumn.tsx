  ...
  return (
    <section className={`flex min-w-[280px] flex-1 flex-col rounded-3xl border bg-white/70 backdrop-blur-xl ${meta.border}`}>
      ...
      <Droppable droppableId={status}>
        {(provided, snapshot) => (
          <div ref={provided.innerRef} {...provided.droppableProps} className={`min-h-[180px] flex-1 space-y-3 rounded-b-3xl p-3 transition ${snapshot.isDraggingOver ? "bg-blue-50/40" : ""}`}>
            {tasks.map((task, index) => (
              <Draggable key={task.id} draggableId={task.id} index={index}>
                {(dragProvided, dragSnapshot) => (
                  <div ref={dragProvided.innerRef} {...dragProvided.draggableProps} {...dragProvided.dragHandleProps} className={`transition-shadow ${dragSnapshot.isDragging ? "rotate-1 scale-[1.02] shadow-xl" : ""}`}>
